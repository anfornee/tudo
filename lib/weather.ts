...
const weatherHeaders = {
  ...
  "User-Agent": process.env.WEATHER_USER_AGENT ?? "(tudo, contact@example.com)",
};
...
async function fetchNws<T>(url: string, revalidate: number): Promise<T> {
...
async function getWeatherPoint(
  ...
  return fetchNws<NwsPointResponse>(
    `${WEATHER_API_URL}/points/${latitude.toFixed(4)},${longitude.toFixed(4)}`,
    60 * 60 * 12,
  );
}
...
async function getCurrentWeather(
  ...
  const stations = await fetchNws<NwsStationCollection>(
  ...
  return {
    ...
    condition: observation.properties.textDescription || "Unknown",
    humidity:
      observation.properties.relativeHumidity.value !== null
        ? Math.round(observation.properties.relativeHumidity.value)
        : null,
    windSpeed: kilometersPerHourToMph(observation.properties.windSpeed.value),
    windDirection:
      observation.properties.windDirection.value !== null
        ? Math.round(observation.properties.windDirection.value)
        : null,
  };
}
...
async function getHourlyForecast(
  ...
  const forecast = await fetchNws<NwsForecastResponse>(
    forecastHourlyUrl,
    60 * 15,
  );
...
async function getDailyForecast(forecastUrl: string): Promise<DailyWeather[]> {
  const forecast = await fetchNws<NwsForecastResponse>(forecastUrl, 60 * 30);
...
export async function getWeather(
  ...
  const locationName =
    city && state ? `${city}, ${state}` : (city ?? "Current location");
